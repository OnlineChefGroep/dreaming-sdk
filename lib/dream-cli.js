...
const TUI_ITEMS = [
  {
    label: "Doctor - inspect plugin and runtime",
    command: "doctor",
    args: [],
    hint: "Check that the dreaming plugin is discovered and the runtime supports it.",
  },
  {
    label: "Test - run deterministic plugin gates",
    command: "test",
    args: [],
    hint: "Run hash determinism and secret-leak gates. Hard failures stop the loop.",
  },
  {
    label: "Eval dry-run - prepare a run plan",
    command: "eval",
    args: ["--dry-run"],
    hint: "Validate the golden corpus and preview the results directory without scoring.",
  },
  {
    label: "Run - test + eval + decisions flow",
    command: "run",
    args: [],
    hint: "Execute the high-level dream flow end to end through the plugin CLI.",
  },
  {
    label: "Status - live pending counts",
    command: "status",
    args: [],
    hint: "Reconcile the live index and report pending session counts.",
  },
  {
    label: "Scope - list in-scope pending sessions",
    command: "scope",
    args: [],
    hint: "List pending sessions that are in scope for the next dream pass.",
  },
  {
    label: "Index - inspect live index summary",
    command: "index",
    args: [],
    hint: "Show the index summary and last dream run for diagnostics.",
  },
  {
    label: "Which - print plugin CLI path",
    command: "which",
    args: [],
    hint: "Print the resolved path to the dreaming plugin CLI entrypoint.",
  },
  { label: "Quit", command: "quit", args: [], hint: "Leave the dream terminal menu." },
];
...
// Backward-compatible plain renderer used for non-TTY output and tests.
function renderTui(model, options = {}) {
  const state = tui.createState(model);
  return tui.renderFrame(
    state,
    { cols: options.cols || 80, rows: options.rows || 24 },
    { color: false, clear: options.clear !== false ? true : false },
  );
}
...
function runTui(parsed, options = {}) {
  ...
  if (!stdin.isTTY || !stdout.isTTY) {
    stdout.write(tui.renderFrame(tui.createState(model), tui.terminalSize(stdout), { clear: false }));
    return model.report.ok ? 0 : 1;
  }

  const result = tui.runInteractive(model, { stdin, stdout });
  if (result.action === "select" && result.item && result.item.command !== "quit") {
    return run([result.item.command, ...(result.item.args || [])], options);
  }
  return 0;
}
